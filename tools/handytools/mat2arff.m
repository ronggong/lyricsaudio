function [ success_value ] = mat2arff( mat_filename, arff_filename, relation, classes )
...
load(mat_filename);
feature_var = featureMatrix; % TODO replace with valid code !! 
...
if isempty(fread(fid))
    ...
    if(length(size(feature_var)) == 2)
        for z=1:size(feature_var,2) % iterate over data set
            ...
            fprintf(fid, '%s\n', AudioType{z}); % new-line at the end of each dataset
        end
    elseif (length(size(feature_var)) == 3)
